...
router.post('/create-order', async (req, res) => {
    try {
        ...
        // 获取服务名称和价格
        let title, totalFee;
        if (serviceType === 'annualReport') {
            title = '企业年报申报服务';
            totalFee = config.prices.annualReport;
        } else if (serviceType === 'zeroReport') {
            title = '年代账服务';
            totalFee = config.prices.zeroReport;
        } else {
            title = '营业执照注销服务';
            totalFee = config.prices.licenseCancel;
        }
